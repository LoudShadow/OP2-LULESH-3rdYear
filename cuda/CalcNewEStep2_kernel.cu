...
//user function
__device__ void CalcNewEStep2_gpu( const double *compHalfStep, const double *delvc, double *q_new, const double *pbvc, double *e_new, const double *bvc,
    ...
    if ( delvc[0] > double(0.) ) {
        ...
        double ssc = ( pbvc[0] * e_new[0]
                + vhalf * vhalf * bvc[0] * pHalfStep[0] ) / m_refdens_cuda ;

        if ( ssc <= m_ssc_thresh_cuda ) {
        ssc = m_ssc_low_cuda ;
        } else {
        ssc = sqrt(ssc) ;
        }
...
__global__ void op_cuda_CalcNewEStep2(
...
void op_par_loop_CalcNewEStep2(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_CalcNewEStep2<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      (double *) arg3.data_d,
