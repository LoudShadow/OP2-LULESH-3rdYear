...
//user function
__device__ void CalcBVC_gpu( double *bvc, const double *compression, double *pbvc) {
...
__global__ void op_cuda_CalcBVC(
  double *arg0,
...
void op_par_loop_CalcBVC(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  CalcBVC");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_CalcBVC<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
