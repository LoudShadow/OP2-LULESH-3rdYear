...
//user function
__device__ void setForceToZero_gpu( double *fx, double* fy, double* fz) {
    fx[0] = 0.0;
    fy[0] = 0.0;
    fz[0] = 0.0;
...
__global__ void op_cuda_setForceToZero(
  double *arg0,
  double *arg1,
  double *arg2,
  int   set_size ) {
...
void op_par_loop_setForceToZero(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_setForceToZero<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
