...
//user function
__device__ void CalcHalfStepBVC_gpu( double *bvc, const double *compression, double *pbvc) {
    bvc[0] = m_c1s_cuda * (compression[0] + double(1.));
    pbvc[0] = m_c1s_cuda;

}

// CUDA kernel function
__global__ void op_cuda_CalcHalfStepBVC(
  double *arg0,
...
void op_par_loop_CalcHalfStepBVC(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  CalcHalfStepBVC");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_CalcHalfStepBVC<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
