...
//user function
__device__ void CheckEOSLowerBound_gpu( const double *vnewc, double *compHalfStep, const double *compression) {
...
__global__ void op_cuda_CheckEOSLowerBound(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    CheckEOSLowerBound_gpu(arg0+n*1,
                       arg1+n*1,
                       arg2+n*1);
  }
}
...
void op_par_loop_CheckEOSLowerBound(char const *name, op_set set,
  ...
  if (OP_diags>2) {
    printf(" kernel routine w/o indirection:  CheckEOSLowerBound");
  }
  ...
  if (set_size > 0) {
    ...
    op_cuda_CheckEOSLowerBound<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      (double *) arg2.data_d,
      set->size );
