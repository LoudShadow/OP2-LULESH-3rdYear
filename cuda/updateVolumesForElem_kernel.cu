...
//user function
__device__ void updateVolumesForElem_gpu( const double *vnew, double *v) {
    double tmpV = vnew[0] ;

    if ( fabs(tmpV - double(1.0)) < m_v_cut_cuda )
    tmpV = double(1.0) ;
...
__global__ void op_cuda_updateVolumesForElem(
  ...
  for ( int n=threadIdx.x+blockIdx.x*blockDim.x; n<set_size; n+=blockDim.x*gridDim.x ){
    ...
    updateVolumesForElem_gpu(arg0+n*1,
                         arg1+n*1);
  }
}
...
void op_par_loop_updateVolumesForElem(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    op_cuda_updateVolumesForElem<<<nblocks,nthread>>>(
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      set->size );
  }
