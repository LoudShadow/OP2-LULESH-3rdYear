...
void CalcNewEStep2_omp4_kernel(
  double *data0,
  int dat0size,
  double *data1,
  int dat1size,
  ...
  for ( int n_op=0; n_op<count; n_op++ ){
    //variable mapping
    const double *compHalfStep = &data0[1*n_op];
    const double *delvc = &data1[1*n_op];
    double *q_new = &data2[1*n_op];
    const double *pbvc = &data3[1*n_op];
    double *e_new = &data4[1*n_op];
    const double *bvc = &data5[1*n_op];
    const double *pHalfStep = &data6[1*n_op];
    const double *ql_old = &data7[1*n_op];
    const double *qq_old = &data8[1*n_op];
    const double *p_old = &data9[1*n_op];
    const double *q_old = &data10[1*n_op];
    ...
    //inline function
      ...
      if ( delvc[0] > double(0.) ) {
          ...
          double ssc = ( pbvc[0] * e_new[0]
                  + vhalf * vhalf * bvc[0] * pHalfStep[0] ) / m_refdens_ompkernel ;
